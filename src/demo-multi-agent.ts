...
import { createWHAgent, type WHAgent } from './agent.js'
...
async function main() {
    ...
    // 创建主 Agent（用于分析和写作阶段）
    const mainAgent = await createWHAgent({
        name: 'Whale Agent - 主智能体',
        skillsDir: '.hi/skills',
        sandbox: {
  ...
  `
    ...
    if (search) {
        ...
        ;[frameworkData, scenarioData] = await Promise.all([
            researcherAgent('LangChain LangGraph Deep Agent framework comparison 2025', search),
            researcherAgent('frontend engineer AI development TypeScript agent 2025', search),
        ])
    } else {
    ...
    const analystAgentInstance = await createWHAgent({
        name: '分析师智能体',
        skillsDir: '.hi/skills',
        sandbox: {
            workspacePath: process.cwd(),
